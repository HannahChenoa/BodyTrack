...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const user = JSON.parse(localStorage.setItem('user', JSON.stringify(data.user)));
    const userId = user ? user.id : null;
    
    if (!userId) {
      document.getElementById('contenedorHistorial').innerHTML = '<p class="text-danger">No se encontró usuario. Inicia sesión.</p>';
      return;
    }
